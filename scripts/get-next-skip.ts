...
(async () => {
  ...
  const unfixedMedium = issues.filter(i => !i.royFix && i.priority === 'Medium' && i.id !== skipId);
  
  if (unfixedMedium.length === 0) {
    console.log('No more unfixed Medium priority issues (excluding skip)!');
    process.exit(0);
  }
  
  const next = unfixedMedium[0];
  console.log(`\n🐛 NEXT MEDIUM PRIORITY BUG (skipping ${skipId}):\n`);
