...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { data: sessionTags, error: sessionTagsError } = await (supabase.from('session_tags') as any)
        .select('tag_id, session:sessions!inner(program_id)')
        .eq('session.program_id', programId);

      if (sessionTagsError) {
        console.warn('[GET /api/tags] session_tags count query failed:', sessionTagsError.message);
      } else if (sessionTags) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { data: templates, error: templatesError } = await (supabase.from('session_templates') as any)
        .select('id, required_skills')
        .eq('program_id', programId);

      if (templatesError) {
        console.warn('[GET /api/tags] session_templates count query failed:', templatesError.message);
      }
      ...
      if (sessionsError) {
        console.warn('[GET /api/tags] sessions count query failed:', sessionsError.message);
      }
      ...
      console.warn('[GET /api/tags] failed to build session counts, returning tags without counts:', countError);
    }
